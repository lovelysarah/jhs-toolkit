...
async function seed() {
    ...
    //     });
    // });
    // const flanders = await db.inventoryLocation.findUniqueOrThrow({
    //     where: { short_id: "MOPn6M699A" },
    // });
    // const joyce = await db.inventoryLocation.findUniqueOrThrow({
    //     where: { short_id: "nFYsxeJ8Kv" },
    // });
    // createItems.forEach(async (item) => {
    //     await db.item.create({
    //         data: {
    //             ...item,
    //             temp_location: "FLANDERS",
    //             last_checked_out_by: "YESS GROUP",
    //             note: "This is a placeholder note",
    //             quantity: Math.floor(Math.random() * 4) + 2,
    //             location: {
    //                 connect: {
    //                     id: Math.random() > 0.5 ? flanders.id : joyce.id,
    //                 },
    //             },
    //         },
    //         // data: {
...
// // const createItems: Pick<Item, "name" | "short_id" | "">[] = [
